...
function increasePrices(percentage) {
  ...
  const realPorcentage = percentage / 100;
  Object.keys(pricesData).forEach((element) => {
    pricesData[element] *= 1 + realPorcentage;
    pricesData[element] = Math.round(pricesData[element] * 100) / 100;
  });
  return pricesData;
}
...
function employeeCoverage(idOrName) {
  ...
  data.employees.forEach((employee) => {
    ...
    allCoverage[`${firstName} ${lastName}`] = employee.responsibleFor.map(
      idAnimal => data.animals.find(animal => animal.id === idAnimal).name,
    );
  });
  ...
  if (idOrName) {
    ...
    return { [fullName]: allCoverage[fullName] };
  }
