...
function entryCalculator(entrants = 0) {
  const price = Object.keys(entrants).map(
    (lifestage) => prices[lifestage]
  );
...
const allLocations = animals
  .map(({ location }) => location)
  .filter((location, index, array) => index === array.indexOf(location));

const animalsFromLocations = allLocations
  .map(aLocation => animals
  .filter(({ location }) => location.includes(aLocation))
  .map(({ name }) => name)
);

function getAnimalsFilteredByLocation() {
  return allLocations.reduce((acc, currentValue, index) => {
    acc[currentValue] = animalsFromLocations[index];
    return acc;
  }, {});
}

function animalMap(options) {
  if (!options) return getAnimalsFilteredByLocation();
}
...
const findEmployeeAnimal = (id) =>
  employees
    .find((employee) => employee.id === id)
    .responsibleFor.find((element) => element);

const getEmployeeAnimal = (id) =>
  animals.find((animal) => animal.id === findEmployeeAnimal(id));

function oldestFromFirstSpecies(id) {
  return Object.values(
    getEmployeeAnimal(id).residents.reduce((acc, currentValue) => {
...
function increasePrices(percentage) {
  ...
  const newValues = Object.values(data.prices).map(
    (price) => (price * 100 + Math.round(price * percentage)) / 100
  );
...
function getEmployeesAnimalsNames(teste) {
  return teste.responsibleFor.map(
